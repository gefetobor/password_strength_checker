...
import com.strength.checker.framework.NetflixScreens;


public class NetflixPasswordStrengthChecker extends Base{

	String NetflixUrl = "NetflixUrl";
	String username;
	String email;
	int n = 5;
	String NetflixPasswordStrength;
	...
	@Test
	public void NetflixPwdStrengthChecker() throws Exception {
		propertiesFileLoad(NetflixUrl);
		NetflixUrl = lobConfigProp.getProperty("NetflixUrl");
		 driver.get(NetflixUrl);
		 driver.manage().window().maximize();
		 driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
         
		
		NetflixScreens executor = new NetflixScreens(driver);
		...
		executor.clickOnGetStarted();
		executor.clickOnNext();
							...
							if (executor.isElementPresent(executor.errorToast)) {
								...
								if (executor.isElementPresent(executor.errorToast)) {
									...
									if (executor.isElementPresent(executor.errorToast)) {
										...
										NetflixPasswordStrength = "Scenarios Not Sufficient. Need more Scenarios to determine Netflix Password Strength";			
										writePasswordStrengthResultToFile(NetflixPasswordStrength);
									}else {
										
										System.out.println("Netflix Password Strength = 52.7");
										NetflixPasswordStrength = "Netflix Password Strength is: 52.7";
										writePasswordStrengthResultToFile(NetflixPasswordStrength);
									}
								}else {
									System.out.println("Netflix Password Strength = 49.1");
									NetflixPasswordStrength = "Netflix Password Strength is: 49.1";
									writePasswordStrengthResultToFile(NetflixPasswordStrength);
								}
							}else {
								System.out.println("Netflix Password Strength = 47.6");
								NetflixPasswordStrength = "Netflix Password Strength is: 47.6";
								writePasswordStrengthResultToFile(NetflixPasswordStrength);
							}
						}else {
							System.out.println("Netflix Password Strength = 45.6");
							NetflixPasswordStrength = "Netflix Password Strength is: 45.6";
							writePasswordStrengthResultToFile(NetflixPasswordStrength);
						}
					}else {
						System.out.println("Netflix Password Strength = 37.6");
						NetflixPasswordStrength = "Netflix Password Strength is: 37.6";
						writePasswordStrengthResultToFile(NetflixPasswordStrength);
					}
				}else {
					System.out.println("Netflix Password Strength = 28.2");
					NetflixPasswordStrength = "Netflix Password Strength is: 28.2";
					writePasswordStrengthResultToFile(NetflixPasswordStrength);
				}
				
			}else {
				System.out.println("Netflix Password Strength = 26.6");
				NetflixPasswordStrength = "Netflix Password Strength is: 26.6";
				writePasswordStrengthResultToFile(NetflixPasswordStrength);
			}
			
		}
		else {
			System.out.println("Netflix Password Strength = 19.9");
			NetflixPasswordStrength = "Netflix Password Strength is: 19.9";
			writePasswordStrengthResultToFile(NetflixPasswordStrength);
		}
